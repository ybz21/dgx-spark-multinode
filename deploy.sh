...
generate_compose() {
    ...
    info "  接口: $node_iface → RDMA: $rdma_hca" >&2
...
cmd_start() {
    ...
    for i in $(seq 0 $((NNODES - 1))); do
        ...
        ssh_node "$i" "cd $DEPLOY_DIR && docker compose --env-file /dev/null -f docker-compose.node${i}.yml up -d"
...
cmd_stop() {
    ...
    for i in $(seq $((NNODES - 1)) -1 0); do
        parse_node "$i"
        info "停止 $HOSTNAME..."
        ssh_node "$i" "cd $DEPLOY_DIR && docker compose --env-file /dev/null -f docker-compose.node${i}.yml down 2>/dev/null" || true
    done
    success "已停止"
}
